...
public class ConnectFour extends JFrame {
            ...
            if (currentState == State.PLAYING) {
            	if (col >= 0 && col < COLS) {
            		...
            		for (int row1 = ROWS -1; row1 >= 0; row1--) {
            			if (board[row1][col] == Seed.NO_SEED) {
            		         board[row1][col] = currentPlayer; // Make a move 
            		         currentState = hasWon(currentPlayer, row1, col);// update state
            		         // Switch player
   ...
   public State hasWon(Seed theSeed, int rowSelected, int colSelected) {
	    ...
	    // Check for 4-in-a-line in the columnSelected
	    count = 0;
	    for (int row = 0; row < ROWS; ++row) {
	    ...
	    // Check for 4-in-a-line on the main diagonal (\ direction)
	    count = 0;
	    ...
	    // Check for 4-in-a-line on the anti-diagonal (/ direction)
	    count = 0;
